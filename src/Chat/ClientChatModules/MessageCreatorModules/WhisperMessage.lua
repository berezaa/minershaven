...
function CreateMessageLabel(messageData, channelName)
	...
	local numNeededSpaces = util:GetNumberOfSpaces(formatPrefix, useFont, useTextSize) + util:GetNumberOfSpaces(formatUseName, useFont, useTextSize) + 2
	...
	if channelName ~= messageData.OriginalChannel then
		...
		NameButton.Position = UDim2.new(0, PrefixLabel.Size.X.Offset + ChannelButton.Size.X.Offset + util:GetStringTextBounds(" ", useFont, useTextSize).X, 0, 0)
		numNeededSpaces = numNeededSpaces + util:GetNumberOfSpaces(formatChannelName, useFont, useTextSize) + 0.5
	else
		local whisperString = messageData.OriginalChannel
		if messageData.FromSpeaker ~= LocalPlayer.Name then
			whisperString = string.format("From %s", messageData.FromSpeaker)
		...
		ChannelButton = util:AddChannelButtonToBaseMessage(BaseMessage, useChannelColor, formatChannelName, messageData.OriginalChannel)
		PrefixLabel.Position = UDim2.new(0, ChannelButton.Size.X.Offset, 0, 0)
		NameButton.Position = UDim2.new(0, ChannelButton.Position + PrefixLabel.Size.X.Offset + ChannelButton.Size.X.Offset + util:GetStringTextBounds(" ", useFont, useTextSize).X, 0, 0)
		numNeededSpaces = numNeededSpaces + util:GetNumberOfSpaces(formatChannelName, useFont, useTextSize) + 0.5
	end
